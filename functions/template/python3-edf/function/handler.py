...
def handle(req):
    ...
    if http_request_mode == 0:
        return handle_single(d, dt, alpha) # single http request for all axis
    else:
        return json.dumps(calculate_single_pid(d, d["current_value"], dt, alpha)) # separate http request for each axis
